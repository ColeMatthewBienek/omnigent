...
def test_host_pid_path_honors_data_dir(tmp_path: Path) -> None:
    """A fresh process places host.pid under ``OMNIGENT_DATA_DIR``.

    The path is an accessor rather than a module constant, so that
    ``OMNIGENT_DATA_DIR`` exported after import still moves it.
    """
