...
export function NewChatLandingScreen() {
  ...
  // Split the picker into "Harnesses" (harness-backed picks — the canonical
  // native terminal CLI wrappers plus generic-ACP harness agents like Grok /
  // Devin / Kilocode) and "Agents" (composed SDK / bundle agents like Polly &
  // Debby plus custom user-registered agents). Harness-backed vs composed, NOT
  // the builtins/customs split: Polly & Debby are built-ins but are composed
  // agents, so they stay under "Agents". A custom agent can run a native
  // harness without being the launcher wrapper, so classify natives by
  // canonical wrapper name rather than harness alone. ACP agents aren't
  // native, so they fold into "More".
  const harnessEntries = useMemo(
    () => agentList.filter((a) => isCanonicalNativeWrapper(a) || isAcpHarnessAgent(a)),
    [agentList],
  );
